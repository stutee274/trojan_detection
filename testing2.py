...
def calculate_features(times, values):
    """Calculate all requested features from signal data"""
    if len(times) < 2:
        return None
    ...
    for filepath in glob.glob(os.path.join(vcd_dir, '*.vcd')):
        ...
        if features:
            features.update({
                ...
                'auto_label': 0  # Default value, can be calculated later
            })
...
def main():
    ...
    if df is not None:
        csv_path = os.path.join(output_dir, 'trojan_signals_features.csv')
        df.to_csv(csv_path, index=False)
        print(f"CSV file saved to: {csv_path}")
        
        # Also save to current directory
        df.to_csv('trojan_signals_features.csv', index=False)
        print("\nResults saved to trojan_signals_features.csv with all requested features")
